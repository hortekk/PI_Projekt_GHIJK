...
namespace Baza
{
    ...
    public class Prijamnice
    {
        ...
        public Prijamnice(DbDataReader dr)
        {
            if (dr != null)
            {
        ...
        public int Spremi()
        {
            ...
            sqlUpit = "INSERT INTO Dokument (idOvlasteneOsobe, idPP, DatumIzdavanja, TipDokumenta) VALUES ('" + idOvlasteneOsobe + "','" + idPP + "','" + DatumIzdavanja + "', 1 )";
        ...
        public static List<Prijamnice> DohvatiDokumente()
        {
            List<Prijamnice> lista = new List<Prijamnice>();
            string sqlUpit = "SELECT * FROM Dokument";
            DbDataReader dr = Baza.Instance.DohvatiDataReader(sqlUpit);
            while (dr.Read())
            {
                Prijamnice dokument = new Prijamnice(dr);
                lista.Add(dokument);
            }
            dr.Close();
            return lista;
